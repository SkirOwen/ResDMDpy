...
def plot_pseudospectra(D, RES, X, Y, x_pts, y_pts):
	...
	ax.set_xlabel(r'$\mathrm{Real \: axis}$')
	ax.set_ylabel(r'$\mathrm{Imaginary \: axis}$')
	fig.tight_layout()
	# TODO: I would like less padding on the right
	plt.savefig("pseudospectra.svg", format="svg")
	plt.show()
...
def plot_eig_res(D, RES2):
	...
	ax.set_xlabel(r'$\mathrm{Real \: axis}$')
	ax.set_ylabel(r"$\mathrm{Angle}$")  # TODO: check this
	plt.savefig("eig_res.svg", format="svg")
	plt.show()
