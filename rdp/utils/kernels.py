...
def kernel_resdmd(xa, ya, xb, yb, cut_off, y2=None, *args) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
	...
	if sketch == Flase:
		...
		s = lambda x: np.max(np.ceil(5 * np.sqrt(m1 + m2) * np.log(m1 + m2)), 5000, np.floor(x))
	...
	if len(I) > n:
		_, I = np.sort(abs(np.diag(d1)), "desc")
	else:
		n = len(I)
		

	p, _, _ = np.linalg.svd(u1[:, I[:N], 0)
	p = u @ sig_dag @ P
	...
	if not sketch:
		...
		psi_xb = np.sqrt(2 / s) * np.cos(th + x.conj().T @ xb)
		psi_yb = np.sqrt(2 / s) * np.cos(th + x.conj().T @ yb)
